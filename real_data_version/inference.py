...
def lds_inference(z_posteriors, params):
    ...
    eta_prior_r = get_resp_weighed_natparams(eta_prior, qu[0])[1]
    eta_trans_r = scan(get_resp_weighed_natparams, eta_trans, qu[1:])[1]
    ...
    qz = jax.tree_multimap(lambda a, b: index_update(a, index[n], b),
                           qz, scan(get_gauss_params, None, qz_natparams)[1])
    qzlag_z = jax.tree_multimap(lambda a, b: index_update(a, index[n], b),
                                qzlag_z, scan(get_gauss_params, None,
                                              qzlag_z_natparams)[1])
...
def inference_fixedpoint(params, posterior):
    # unpack
    eta_hmm, eta_prior, eta_transition, eta_likelihood = params
    qz, qzlag_z, qu, quu = posterior
    ...
    qu, quu = vmap(hmm_inference)(
                   (eta_hmm, eta_prior, eta_transition, qz, qzlag_z))
    return (qz, qzlag_z, qu, quu)

def OLDmake_inference(x, C, R, b_prior, Q_prior, B, b, Q, pi, A):
    # transform into natural parameter form
    N = x.shape[0]
    hmm_natparams = scan(get_hmm_natparams, None, (pi, A))[1]
    prior_natparams = vmap(lambda a, b: scan(get_prior_natparams, None,
                           (a, b))[1])(b_prior, Q_prior)
    transition_natparams = vmap(lambda a, b, c:
                                scan(get_transition_natparams,
                                     None, (a, b, c))[1])(B, b, Q)
    likelihood_natparams = get_likelihood_natparams(C, R, x)

    #@jit
    def OLDinference(posterior, iteration):
        # unpack
        qz, qzlag_z, qu, quu = posterior

        # run inference
        qz, qzlag_z = scan(lds_inference,
                           (qz, qzlag_z),
                           (prior_natparams, transition_natparams,
                            likelihood_natparams, qu, jnp.arange(N)))[0]

        qu, quu = vmap(hmm_inference)(
                       (hmm_natparams, prior_natparams, transition_natparams,
                        qz, qzlag_z))
        return (qz, qzlag_z, qu, quu), None
    return OLDinference
...
if __name__ == "__main__":
    ...
    # run inference
    inference_runner = OLDmake_inference(x, C, R, b_prior, Q_prior,
                                         B, b, Q, pi, A)
    qz, qzlag_z, qu, quu = scan(inference_runner, (qz, qzlag_z, qu, quu),
                                jnp.arange(inference_iters))[0]

    corr_to_true, _, _ = matching_sources_corr(qz[0][:, :, 0], z_mu[:, :, 0])
    print(corr_to_true)
