...
def emission_likelihood(params, input_data, mu_est, D_est):
    ...
    s_est = s_est[:, :int(s_est.shape[1] / 2)]
    x0 = input_data[0, int(input_data.shape[1] / 2):]
    logpx0 = jscipy.stats.multivariate_normal.logpdf(x0,
                                                     jnp.zeros(x0.shape),
                                                     jnp.diag(jnp.ones(x0.shape)))

    # calculate emission probabilities using current parameters
    T = input_data.shape[0]
    K = mu_est.shape[0]
    logp_s = jnp.zeros(shape=(T, K))
    for k in range(K):
        lpx_per_k = jscipy.stats.multivariate_normal.logpdf(s_est,
                                                            mu_est[k],
                                                            D_est[k])
        logp_s = ops.index_update(logp_s, ops.index[:, k],
                                        lpx_per_k)
    logp_J = J_loglikeli_contrib(params, input_data)
    return logp_s, logpx0, logp_J, s_est
...
def forward_backward_algo(logp_x, transition_matrix, init_probs):
    ...
    eps = 1e-30
    x_probs = jnp.clip(x_probs, a_min=eps)
