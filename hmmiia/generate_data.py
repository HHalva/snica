...
def gen_source_data(num_comp, num_latent_states, sequence_length,
    ...
    means = -np.ones((num_latent_states, num_comp))
    combs = list(combinations(np.arange(num_comp), 2))
    # towhere = onp.random.permutation(onp.arange(num_latent_states))[:len(combs)]
    towhere = np.arange(len(combs))
    for i in range(len(combs)):
        means[towhere[i], combs[i][0]] = 1
        means[towhere[i], combs[i][1]] = 1
    means = means - np.mean(means, axis=0)

    sigmas = 0.25 * np.ones((num_latent_states, num_comp))
    towhere = np.random.permutation(np.arange(num_latent_states))[:len(combs)]
    for i in range(len(combs)):
        sigmas[towhere[i], combs[i][0]] = 1
        sigmas[towhere[i], combs[i][1]] = 1
    sigmasq = sigmas**2
