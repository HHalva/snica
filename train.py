...
def train(x, cfg):
    ...
    a0 = jnp.tile(jnp.log(jnp.ones(K) / K), (N, 1))
    A = jnp.ones((K, K)) / K + jnp.eye(K)
    A = jnp.tile(jnp.log(A / A.sum(1)), (N, 1, 1))

    # initialize SLDS params
    key, *keys_slds = jax.random.split(key, 6)
    b0 = jax.random.uniform(keys_slds[0], (N, K, d), minval=-.5, maxval=.5)
    b = jax.random.uniform(keys_slds[1], (N, K, d), minval=-1, maxval=1)
    B = jax.random.uniform(keys_slds[2], (N, K, d, d), minval=-1, maxval=1)
    # should below be in logs?
    Q0 = jax.vmap(
        lambda _: jnp.sqrt(
        jnp.diag(1 / jax.random.uniform(_, (d,), minval=.5, maxval=1.))
        )
    )(jax.random.split(keys_slds[3], N * K)).reshape((N, K, d, d))
    Q = jax.vmap(
        lambda _: jnp.sqrt(
        jnp.diag(1 / jax.random.uniform(_, (d,), minval=.5, maxval=1.))
        )
